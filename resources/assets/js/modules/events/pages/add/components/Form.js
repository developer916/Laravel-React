...
const displayName = 'AddEventForm';
const propTypes = {
...
const Form = ({ event, calendars, errors, onSubmit, onChange, reminders, units, addNotification, removeNotification, handleNotification, t , getError }) => {
  ...
  function renderCalendars() {
    ...
    return (
      <Input className="float-right actions-select" type="select" name="calendarId" onChange={(e)=> handleChange(e.target.name, e.target.value)} required>
  ...
  return (<form className="event-form" onSubmit={e => onSubmit(e)}>
    <Card>
      <CardBody className="p-4">
        <h1><Trans i18nKey="events.add_event"/></h1>
        <Row>
          <Col xs="12">
              {(() => {
                  if(getError == true) {
                      return (
                          <div className="alert alert-danger" role="alert" style={{marginTop: '15px'}}>
                            <Trans i18nKey="events.can_not_create_new_event"/>
                          </div>
                      )
                  }
              })()}
        ...
        <div className="mb-3 clearfix">
          <Label><Trans i18nKey="events.select_the_calendar"/>:&nbsp;&nbsp;</Label>
          { renderCalendars() }
        </div>
        <FormGroup>
               ...
               <FormGroup>
                 ...
                 <Col md="3" style={{paddingLeft: 0}}>
                   <DatetimePickerTrigger
                       ...
                       startDay={0}
                       weeks={['Mon', 'Die', 'Mit', 'Don', 'Fri', 'Sam', 'Son']}
                       months={['Jan', 'Feb', 'Mär', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez']}>
