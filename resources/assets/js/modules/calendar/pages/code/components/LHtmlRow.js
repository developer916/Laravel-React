...
class LHtmlRow extends Component {
  ...
  render() {
    const {calendar, selected, rowData} = this.props

    let lhtmlColumns = rowData.data.map(each => {
      return (
        <LHtmlColumn columnData={each} key={each.key} calendar={calendar}/>
      )
    });
    ...
    return (<div className={lhtmlRowClassName} style={rowData.style}  onClick={() => this.selectElement(rowData)}>
      {lhtmlColumns}
      <Button outline color="primary" onClick={() => this.addButton()} className="lhtml-add-button"><i className="fa fa-plus fa-lg"></i>&nbsp;<Trans i18nKey="calendars.add_element"/></Button>
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslation()(LHtmlRow));